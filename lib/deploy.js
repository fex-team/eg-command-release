...
function uploadBCS(bcsOpt, to, release, content, subpath, callback){
    bcs.upload(
        //url, request options, post data, file
        bcsOpt, content, to + release,
        function(err, res){
            if(err || res.trim() != '0'){
                fis.log.error('upload file [' + subpath + '] to [' + to +
...
function deploy(dest, file, callback){
    if(file.release){
        ...
        if(dest.receiver){
            ...
            if(!file.useHash || dest.opt.md5 != 1){
                uploadBCS(dest.bcs, dest.to, release, content, file.subpath, callback);
            }
            if(file.useHash && dest.opt.md5 > 0){
                uploadBCS(dest.bcs, dest.to, file.getHashRelease(release), content, file.subpath, callback);
            }
        } else {
